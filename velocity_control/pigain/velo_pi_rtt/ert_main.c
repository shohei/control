...
void rt_OneStep(void)
{
  ...
  {                                    /* Sample time: [0.0s, 0.0s] */
    rtExtModeUpload(0, velo_pi_M->Timing.t[0]);
  }

  {                                    /* Sample time: [0.02s, 0.0s] */
    rtExtModeUpload(1, ((velo_pi_M->Timing.clockTick1) * 0.02));
  }
...
int_T main(void)
{
  ...
  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(velo_pi_M));
  rtExtModeCheckInit(2);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(velo_pi_M->extModeInfo, 2, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(velo_pi_M, true);
    }
  }
  ...
  /* The main step loop */
  while ((rtmGetErrorStatus(velo_pi_M) == (NULL)) && !rtmGetStopRequested
         (velo_pi_M)) {
    /* External mode */
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(velo_pi_M->extModeInfo, 2, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(velo_pi_M, true);
      }
    }
