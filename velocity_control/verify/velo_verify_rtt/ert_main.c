...
#include <ext_share.h>
#include <updown.h>
#define INIT_TIMER_VAL                 43314
#define SETUP_PRESCALER                TCCR5B |= ((1<<CS51)); TCCR5B &= ~((1<<CS52) | (1<<CS50))
...
#define SCHEDULER_COUNTER_TARGET       3
...
void rt_OneStep(void)
{
  ...
  {                                    /* Sample time: [0.0s, 0.0s] */
    rtExtModeUpload(0, velo_verify_M->Timing.t[0]);
  }

  {                                    /* Sample time: [0.033333333333333333s, 0.0s] */
    rtExtModeUpload(1, ((velo_verify_M->Timing.clockTick1) *
...
int_T main(void)
{
  ...
  velo_verify_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(velo_verify_M));
  rtExtModeCheckInit(2);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(velo_verify_M->extModeInfo, 2, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(velo_verify_M, true);
    }
  }
  ...
  /* The main step loop */
  while ((rtmGetErrorStatus(velo_verify_M) == (NULL)) && !rtmGetStopRequested
         (velo_verify_M)) {
    /* External mode */
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(velo_verify_M->extModeInfo, 2, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(velo_verify_M, true);
      }
    }
