...
ts = 1/50;
u_ini = 1.5;
u_ref_list = 1.8:0.2:3;
open_system('velo_gain.slx');
open_system('velo_gain/Scope');
y_mean_list = [];
for i=1:length(u_ref_list)
    u_ref = u_ref_list(i);
    set_param('velo_gain', 'SimulationCommand', 'start');
    while(1)
        stat = get_param('velo_gain','SimulationStatus');
