...
void rt_OneStep(void)
{
  ...
  {                                    /* Sample time: [0.033333333333333333s, 0.0s] */
    rtExtModeUpload(0, velo_M->Timing.taskTime0);
  }
...
int_T main(void)
{
  ...
  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(velo_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(velo_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(velo_M, true);
    }
  }
  ...
  /* The main step loop */
  while ((rtmGetErrorStatus(velo_M) == (NULL)) && !rtmGetStopRequested(velo_M))
  {
    /* External mode */
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(velo_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(velo_M, true);
      }
    }
