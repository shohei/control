...
void rt_OneStep(void)
{
  ...
  {                                    /* Sample time: [0.0s, 0.0s] */
    rtExtModeUpload(0, velo_gain_M->Timing.t[0]);
  }

  {                                    /* Sample time: [0.01s, 0.0s] */
    rtExtModeUpload(1, ((velo_gain_M->Timing.clockTick1) * 0.01));
  }
...
int_T main(void)
{
  ...
  velo_gain_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(velo_gain_M));
  rtExtModeCheckInit(2);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(velo_gain_M->extModeInfo, 2, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(velo_gain_M, true);
    }
  }
  ...
  /* The main step loop */
  while ((rtmGetErrorStatus(velo_gain_M) == (NULL)) && !rtmGetStopRequested
         (velo_gain_M)) {
    /* External mode */
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(velo_gain_M->extModeInfo, 2, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(velo_gain_M, true);
      }
    }
