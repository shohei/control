...
void rt_OneStep(void)
{
  ...
  {                                    /* Sample time: [1.0s, 0.0s] */
    rtExtModeUpload(0, pos_test_M->Timing.taskTime0);
  }
...
int_T main(void)
{
  ...
  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(pos_test_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(pos_test_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(pos_test_M, true);
    }
  }
  ...
  /* The main step loop */
  while ((rtmGetErrorStatus(pos_test_M) == (NULL)) && !rtmGetStopRequested
         (pos_test_M)) {
    /* External mode */
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(pos_test_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(pos_test_M, true);
      }
    }
