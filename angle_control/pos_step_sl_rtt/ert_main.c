...
void rt_OneStep(void)
{
  ...
  {                                    /* Sample time: [0.02s, 0.0s] */
    rtExtModeUpload(0, pos_step_sl_M->Timing.taskTime0);
  }
...
int_T main(void)
{
  ...
  pos_step_sl_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(pos_step_sl_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(pos_step_sl_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(pos_step_sl_M, true);
    }
  }
  ...
  /* The main step loop */
  while ((rtmGetErrorStatus(pos_step_sl_M) == (NULL)) && !rtmGetStopRequested
         (pos_step_sl_M)) {
    /* External mode */
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(pos_step_sl_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(pos_step_sl_M, true);
      }
    }
